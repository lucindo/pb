...
describe('session primary action label', () => {
  it('maps action ids to localized control copy', () => {
    expect(getSessionPrimaryActionLabel('start', UI_STRINGS.en.controls)).toBe('Start session')
    expect(getSessionPrimaryActionLabel('end', UI_STRINGS.en.controls)).toBe('End session')
    expect(getSessionPrimaryActionLabel('cancel', UI_STRINGS['pt-BR'].controls)).toBe('Cancelar')
  })
})
