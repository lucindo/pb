...
/**
 * D-04/D-05: Resolves the given ThemeId to a favicon data-URI and sets the <link rel="icon"> href.
 ...
 * - null-guarded on the <link> element (jsdom test host may not have one — same posture as useTheme.ts:43).
 */
function applyFavicon(theme: ThemeId): void {
  ...
  // null-guard: jsdom test host may not have the <link rel="icon"> element.
...
export function useFavicon(): void {
  ...
  useEffect(() => {
    ...
    const onChange = (event: MediaQueryListEvent): void => {
      ...
      link?.setAttribute('href', buildFaviconDataUri(resolved))
    }
