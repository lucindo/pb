...
describe('FeedbackTime', () => {
  it('renders the primary value (e.g. remaining time)', () => {
    render(<FeedbackTime primary="02:51" secondary="5.5 BPM · 1:1" ariaLabel="Pattern Breathing readout" />)
  ...
  it('exposes a readout region named by ariaLabel with aria-live disabled (timer ticks would flood SR)', () => {
    render(<FeedbackTime primary="00:00" secondary="—" ariaLabel="Live Pattern Breathing time" />)
    const status = screen.getByRole('status', { name: 'Live Pattern Breathing time' })
    expect(status).toBeVisible()
    expect(status).toHaveAttribute('aria-live', 'off')
  })

  it('places the primary value above the secondary caption (DOM order)', () => {
    render(<FeedbackTime primary="12:34" secondary="6 BPM" ariaLabel="Pattern Breathing readout" />)
    const status = screen.getByRole('status', { name: 'Pattern Breathing readout' })
