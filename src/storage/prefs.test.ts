...
describe('coercePrefs (D-10 / D-17)', () => {
  ...
  it('preserves all valid fields verbatim', () => {
    const valid: UserPrefs = { ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: 'pt-BR' }
    expect(coercePrefs(valid)).toEqual(valid)
  })

  it('falls back PER FIELD when theme is invalid (D-17) — keeps timbre + locale', () => {
    expect(coercePrefs({ theme: 'neon', timbre: 'bowl', locale: 'en' }))
      .toEqual({ ...DEFAULT_PREFS, theme: DEFAULT_THEME, timbre: 'bowl', locale: 'en' })
  })

  it('falls back PER FIELD when timbre is invalid (D-17) — keeps theme + locale', () => {
    expect(coercePrefs({ theme: 'dark', timbre: 'trumpet', locale: 'en' }))
      .toEqual({ ...DEFAULT_PREFS, theme: 'dark', timbre: DEFAULT_TIMBRE, locale: 'en' })
  })

  it('falls back PER FIELD when locale is invalid (D-17) — keeps theme + timbre', () => {
    expect(coercePrefs({ theme: 'dark', timbre: 'bell', locale: 'pt_BR' }))
      .toEqual({ ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: DEFAULT_LOCALE })
  })
  ...
  it('tolerates legacy variant key on persisted envelope — VAR-05 forward-compat (Phase 38 D-01)', () => {
    ...
    const legacyDiamondEnvelope: unknown = { theme: 'dark', timbre: 'bell', locale: 'pt-BR', variant: 'diamond' }
    const coercedDiamond = coercePrefs(legacyDiamondEnvelope)
    expect(coercedDiamond).toEqual({ ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: 'pt-BR' })
  ...
  it('does not throw when raw has prototype-polluting keys (T-25-01 mitigation)', () => {
    // Prototype-pollution mitigation: we only read known keys, never spread `raw`
    // into an object we use as a prototype. Test that a __proto__ key in the raw doesn't
    // propagate to the returned object.
    const polluted: unknown = JSON.parse('{"theme":"system","timbre":"bowl","locale":"en","__proto__":{"polluted":true}}')
...
describe('coerceTheme / coerceTimbre / coerceLocale (D-10 per-field)', () => {
...
describe('coercePrefs corrupt-field tolerance (Phase 47 PREFS-04)', () => {
  ...
  it("bypassSilentMode: 'junk' → true default (D-05); other fields preserved", () => {
    const out = coercePrefs({
      theme: 'dark', timbre: 'bell', locale: 'pt-BR',
  ...
  it("pre-flags envelope (3 keys only) coerces the new key to its per-flag default (PREFS-03)", () => {
    // A returning user from a pre-flags build has 3 prefs keys — bypassSilentMode
    // is undefined and its coercer falls back to the per-flag default.
    // Returning users see the production defaults.
    const out = coercePrefs({ theme: 'dark', timbre: 'bell', locale: 'pt-BR' })
    expect(out).toEqual({
      ...DEFAULT_PREFS,
      theme: 'dark', timbre: 'bell', locale: 'pt-BR',
    })
...
describe('loadPrefs / savePrefs round-trip', () => {
  ...
  it('round-trips a valid UserPrefs object', () => {
    const next: UserPrefs = { ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: 'pt-BR' }
    savePrefs(next)
    expect(loadPrefs()).toEqual(next)
  })

  it('round-trips a 4-field UserPrefs with the flag set to a non-default value (Phase 47 + 49.1)', () => {
  ...
  it('preserves settings, mute, and stats fields when saving prefs (envelope merge)', () => {
    ...
    savePrefs({ ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: 'pt-BR' })
  ...
  it('does not throw when underlying setItem throws (D-16)', () => {
    ...
    expect(() => { savePrefs({ ...DEFAULT_PREFS, theme: 'dark', timbre: 'bell', locale: 'pt-BR' }) }).not.toThrow()
  })
...
describe('DEFAULT_PREFS shape', () => {
  ...
  it('DEFAULT_PREFS has the 4 contracted fields with their defaults', () => {
...
describe('THM-05 forward-compat (CONTEXT D-02)', () => {
  ...
  it('coerces deprecated persisted theme values to "system" on read — THM-05 (CONTEXT D-02)', () => {
    for (const deprecated of ['moss', 'slate', 'dusk']) {
      window.localStorage.setItem(STATE_KEY, JSON.stringify({
        version: 1,
        prefs: { theme: deprecated, timbre: 'bowl', locale: 'en' },
      }))
