...
//
// The AudioContext is created once on the first preview tap and reused for every
// subsequent tap. Construction cost (~5-20 ms) is paid once; all following taps
// skip straight to scheduling. Sharing or closing the context between taps would
// reintroduce that latency.
//
// Some browsers auto-suspend an idle AudioContext. Calling ctx.resume() before
// scheduling on every tap closes that gap. No await — Web Audio tolerates a
// same-microtask resume + schedule pair, and awaiting would break the
// synchronous-call-path contract.
//
// phaseDurationSec is deliberately omitted from the scheduleInCueForTimbre call.
// Each preset's own decayTau values define the audition shape; BPM / session
// duration coupling is explicitly rejected.
...
export function playInhalePreview(timbre: TimbreId): void {
  ...
  if (previewCtx.state === 'suspended') {
    // Fire-and-forget — no await (synchronous-call-path contract).
