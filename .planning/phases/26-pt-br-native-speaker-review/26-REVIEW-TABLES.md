...
| `breathing.inhale` | `In` | `Puxa` | `Puxa` | KEPT | Operator override: UI constraint — this label renders inside the breathing shape and must stay as short as `In`/`Out` or the layout breaks. "Inspira" is more precise but too long. Keep the short colloquial "Puxa". |
| `breathing.exhale` | `Out` | `Solta` | `Solta` | KEPT | Operator override: UI constraint — must stay as short as `In`/`Out` or the layout breaks. "Expira" too long. Keep the short colloquial "Solta". |
