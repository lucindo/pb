...
key_files:
  ...
  modified:
    - src/app/App.tsx
decisions:
  - "iOS detection lives in `src/hooks/useIsStandaloneOrPhone.ts` via a `detectIsIOS()` user-agent probe (`/iP(hone|od|ad)/.test(navigator.userAgent)` plus the iPadOS 13+ `Macintosh` + `maxTouchPoints > 1` branch), computed once per page lifetime in a lazy `useState` initializer and returned as `isIOS` alongside `isStandalone`/`isPhone` (CR-01/WR-01 superseded the original `(navigator as SafariNavigator).standalone !== undefined` per-render probe — `.standalone` is unreliable on iPadOS 13+ desktop-mode Safari). The local `SafariNavigator` interface is still declared in the same hook for the `isStandalone` branch's `.standalone === true` check."
