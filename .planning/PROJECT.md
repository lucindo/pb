...
- **Timbre preview cue (Phase 40)** — switching Timbre in App Settings plays the inhale-only cue at current pitch via the existing scheduler. **Complete (2026-05-21):** PREV-01..05 closed (4 verified structurally, 4 empirical items pending in `40-HUMAN-UAT.md`), 1093/1093 tests pass, PREV-03 mute-irrelevance locked structurally by `previewContext.no-audioengine-import.test.ts` drift-guard at the import-graph level.
- **Spike 010 Mono Zen — full implementation (Phase 41)** — **Complete (2026-05-25):** spike-loop format (per-item propose/go/implement/approve cycle, items J1-J18, ~100+ atomic commits) **absorbed the originally-planned Phases 41 (palette+tokens), 42 (new orb), and 43 (five-surface redesign) into a single tightly-coupled implementation.** Delivered: Mono Zen light + dark palettes (cool slate) with new `borderSoft` / `textSoft` / `orbHalo1/2/3` / `onAccent` tokens; self-hosted Inter Variable typography; 3-halo + centre disc orb with V1/V2 variants behind **query-string flags** (operator deviation — `?breathingShape=` + `?orbIdle=`, not `VITE_*`); idle orb states (still + ambient 40:60); SetupCard + SettingsSheet (responsive bottom-sheet/center-modal) + FeedbackTime + FeedbackCount primitives; 4-section App Settings page (Appearance / Language / Feedback / About — "Audio" renamed); Complete screen kept with checkmark orb + "Session complete · Take a moment" + Done; Learn restructured (SectionHeader + SectionCard pattern); desktop centered column (520/600/320 px); no-jiggle invariant; LOCKED_COPY verified verbatim across redesign (J17); J18 final audit (8-item orphan sweep + drift-guard `content.no-removed-keys.test.ts`). 107 files / 1155 tests pass; PWA precache 514.18 KiB. **Operator decisions:** install banner V3 (UX-12/13/14) DROPPED entirely per J13 — install stays only in App Settings; theme picker keeps 3 options (Light/Dark/System) — operator validated all 3 during design. Closed at commit `d2b886b`. See `.planning/phases/41-spike-mono-zen/`.
- **Final POLISH (Phase 44)** — **Complete (2026-05-25):** POLISH-01..09 closed across 7 plans/waves. 44-01 mega-commit `476caba` ran `/gsd:code-review --all --fix` (lint 0/0 on HEAD; 28 Info findings dispositioned in `44-INFO-FINDINGS.md` — 3 fix, 24 obsolete-by-redesign after Phase 41 J1-J18 rewrites, 1 deferred). 44-02 `dac3dec` tightened test names + removed intra-file redundancy (1155→1153 tests, 0 token-locking, 7 drift-guards preserved, 3× flake-clean). 44-03 `4a0b77f` broad WHY-only comment sweep (19 DROP / 75 KEEP across 13 files, zero behavior change per Item I sibling-pattern mechanic). 44-04 `b84f936` extracted `SettingsRow` primitive (3 adapters retrofitted, +3 behavioral tests) and reconciled the obsolete `.orb-layer` rename (deleted in Phase 41 J4 `a742c0b`). 44-05 fold case (POLISH-07 already closed by 44-03; 20 grep hits all KEEP-with-rationale — WebAudio `square`, Mono Zen "cool slate", AUDIO-02 `chime` migration guard). 44-06 `e6b2f24` `44-SECURITY.md` (22 STRIDE threats, status `verified`, 0 open: 11 inherited from `40-SECURITY.md` + 11 new for query-string toggles + font asset). 44-07 `e4ff788` `44-VERIFICATION.md` (9/9 POLISH satisfied, status `passed`) + closed REFACTOR-LOOP-STATE.md bookkeeping leak per D-12 (F6 `fe14c47` + I `80da948` marked approved). 1156 tests pass; lint 0/0; per-commit green-gate held through milestone close (POLISH-09); zero net-new runtime code deps (POLISH-08 — react + react-dom only; fontsource as asset).
- **Ring progress-cue toggle (Phase 45)** — **Complete (2026-05-25):** 3 plans across 3 sequential waves. 45-01 added the `RingCueStyle` type + `RING_CUE_FLAG` query-string spec + `FeatureFlags.ringCue` field, mirroring the existing `BREATHING_SHAPE_FLAG` / `ORB_IDLE_FLAG` pattern. 45-02 added the `ringCue` prop to `OrbShape` and branched the inner-ring slot to a verbatim spike-011 `ProgressArcLayer` transcription (viewBox 0 0 100 100, r = 49.7, sweep-flags 0/1, strokeWidth 2.5, no `pathLength`/`stroke-dasharray`; reduced-motion suppresses the arc but the faint outer track survives). 45-03 threaded `vm.featureFlags.ringCue` through `PracticeScreen → PracticeSessionView → {Breathing,NaviKriya}SessionSurface → OrbShape`. **Default flipped to `progress-arc` post-UAT at operator request** (`feat(45): flip ringCue default to progress-arc post-UAT`) — the prior outer + inner ring rendering remains reachable via `?ringCue=outer-inner` (with aliases `production` / `rings` / `default`). 1166/1166 tests pass; tsc 0; lint 0; PWA precache 515.72 KiB. Both visual UAT items operator-approved (`45-HUMAN-UAT.md`). VERIFICATION: 14/14 must-haves verified programmatically.
...
- **Dropped permanently** — Firefox Desktop orb scale-animation flicker (the orb was rebuilt in Phase 41 via spike-loop J4 — old `.orb` keyframes are gone).
...
- **Naturally superseded by redesign** — Inner-ring UX symmetry (replaced by Phase 41 J4 orb rewrite); Code review WR-01 `IosInstallSteps` `::marker` (install banner V3 DROPPED entirely in Phase 41 J13 — install stays only in App Settings, where `IosInstallSteps` still consumes its own theme-aware copy); 28 Info-severity findings from 2026-05-16 (to be swept by Phase 44 POLISH).
...
---
*Last updated: 2026-05-25 after Phase 45 (Ring progress-cue toggle) completion — default flipped to `progress-arc` post-UAT*
