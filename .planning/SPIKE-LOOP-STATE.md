...
- C — Cue Style: PickerCardGrid columns extended to support 4; CuePicker columns 3 → 4 (matches spike VisualPicker L1564). Existing CueGlyph component kept per operator's "visual reference, NOT content" note. Reverted to 3-col in commit `1d0f506` (app domain has only 3 cue options; 4-col created empty cell + off-center pills). **DONE — commit `1d0f506`**
- D — Timbre: per-option Unicode glyph (◯ Bowl / ⟡ Bell / ∿ Sine / ⌇ Flute) inline with label; aria-hidden so the radio's accessible name stays as the localized label. TimbrePicker.test.tsx 2 tests updated from textContent equality to accessible-name queries. **DONE — commits `035e099` + `2a0cf9b` (bell/flute glyph swap)**

**Subsequent feedback items (chronological):**
- Segmented row label left-align (`e45f07e`)
- Top-bar pt-50 → safe-area-aware spacing (`a9e4767`)
- Learn page → SectionHeader + SectionCard pattern (`556463f`, `7f1d10e` operator-correction to per-section eyebrows, `c0347ba` text-sm body)
- EndSessionDialog re-tokenized (`2640981`)
- Stretch SetupCard → 3-cell summary (`167f536`)
- Navi SetupCard short labels (`8fdc43a`)
- Practice settings sheet — Stepper + SegmentedRow + ToggleRow chrome (`7226b3e`)
- Open-ended → ∞ (`3171509`)
- HRV running inline Duration stepper (`ab3a58b`), then reverted to no-UI for congruence (`60531ea`)
- Session complete — checkmark orb + "Take a moment" + Done button (`afe45eb`)
- Arrow cue Mobile Safari GPU layer cache fix (`c4db085`)
- "Audio" section → "Feedback" (`4ab2776`)
- Sheet mobile max-h 58vh → 85vh (`3eb1760`)
- Ambient idle orb → 40:60 inhale:exhale (`a70fd22`)
- About row appends build SHA + ISO date (`a9501fc`)
