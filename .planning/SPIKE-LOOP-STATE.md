...
3. Read `.planning/spikes/010-mono-zen-light-dark/README.md` — search for "Desktop", "desktop", "centered column", "520", "600", "320" for desktop layout decisions
4. Read current code:
   - `src/components/primitives/PageShell.tsx` — does it already cap width?
   - `src/app/PracticeScreen.tsx` — current responsive structure
   - `src/app/pages/AppSettingsPage.tsx` / `LearnPage.tsx` — current responsive structure
   - CSS variables: any existing orb-size or column-width tokens?
5. Apply the propose-step checklist. Scope:
   - Practice column max-width 520px (centered) on desktop (≥ sm)
   - Learn / AppSettings column max-width 600px (centered) on desktop
   - Orb size: 320px diameter on desktop (current is mobile-driven)
   - Settings sheet: already responsive via J9 (sm: center modal). Verify it still works.
   - Likely small CSS / Tailwind class changes; no domain
