...
| `src/components/` | UI: `OrbShape` (single `minimal-rings` guide) + readouts (`SessionReadout`, `FeedbackTime`), `PatternBreathingSettingsForm` + `Settings*` form parts, `SettingsSheet`, `SettingsStatsSection`, dialogs, `LearnAnchor`/`LearnPanel`, install steps, Theme/Timbre/Language pickers. `primitives/`, `icons/`. |
